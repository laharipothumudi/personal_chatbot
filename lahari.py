...
st.markdown('<h1 class="chat-font">🤖 Pothumudi Lahari Clone Chatbot</h1>', unsafe_allow_html=True)
st.markdown('<h3 class="chat-font">Ask me anything, and I\'ll respond as Pothumudi Lahari!</h3>', unsafe_allow_html=True)
st.markdown("---")
...
def generate_refined_answer(query, retrieved_answer):
    prompt = f"""You are Pothumudi Lahari, an AI, DS Student. Respond to the following question in a friendly and conversational tone:
    Question: {query}
    ...
    with st.spinner("Thinking..."):
        try:
            ...
            if retrieved_answer:
                ...
                response = f"**Pothumudi Lahari**:\n{refined_answer}"
            else:
                response = "**Pothumudi Lahari**:\nI'm sorry, I cannot answer that question."
        except Exception as e:
